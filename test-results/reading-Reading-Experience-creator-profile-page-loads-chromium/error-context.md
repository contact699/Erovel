...
- Name: reading.spec.ts >> Reading Experience >> creator profile page loads
- Location: e2e\reading.spec.ts:107:7
...
Locator: getByText('Stories')
Expected: visible
Error: strict mode violation: getByText('Stories') resolved to 4 elements:
    1) <a href="/browse?format=chat" class="text-muted hover:text-foreground transition-colors">Chat Stories</a> aka getByRole('navigation').getByRole('link', { name: 'Chat Stories' })
    2) <p class="text-xs text-muted leading-relaxed">A platform for adult fiction creators and readers…</p> aka getByText('A platform for adult fiction')
    3) <a href="/browse" class="hover:text-foreground transition-colors">All Stories</a> aka getByRole('link', { name: 'All Stories' })
    4) <a href="/browse?format=chat" class="hover:text-foreground transition-colors">Chat Stories</a> aka getByRole('contentinfo').getByRole('link', { name: 'Chat Stories' })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('Stories')
